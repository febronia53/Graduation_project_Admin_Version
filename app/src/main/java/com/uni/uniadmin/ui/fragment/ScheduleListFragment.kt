...
class ScheduleListFragment : Fragment() {
    ...
    lateinit var scheduleDataType:MutableList<ScheduleDataType>
        ...
        savedInstanceState: Bundle?
                ...
                val adapterDepartment: ArrayAdapter<CharSequence> = ArrayAdapter.createFromResource(requireContext(),R.array.departement,R.layout.spinner_item)
                ...
                autoCom.onItemSelectedListener= object : AdapterView.OnItemSelectedListener{
                    override fun onItemSelected(p0 : AdapterView<*>?, p1: View?, p2:Int, p3: Long) {
                        ...
                        binding.departmentTextSchedule.text=department}
                      ...
                      if(item.type ==  ScheduleAdapter.VIEW_TYPE_ONE){
                          ...
                          if (department.isNotEmpty()){
                              viewModel.deleteLecture(
                                  Lecture(item.eventId,item.courseID,"","","","","","",false),department)
                              observeDeletedLecture()
                          }else{
                              Toast.makeText(context,"make sure to chose the departement",Toast.LENGTH_SHORT).show()
                          }
                           }
                    })
